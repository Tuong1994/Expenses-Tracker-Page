...
export const getCategories = async (query: ApiQuery) => {
  const response = await FetchServer.Get<List<Category>>(categoryApiPaths.list + getApiQuery(query));
  return response;
};

export const getCategoriesPaging = async (query: ApiQuery) => {
  const response = await FetchServer.Get<Paging<Category>>(categoryApiPaths.listPaging + getApiQuery(query));
  return response;
};

export const getCategory = async (query: ApiQuery) => {
  const response = await FetchServer.Get<Category>(categoryApiPaths.detail + getApiQuery(query));
  return response;
};

export const createCategory = async (data: CategoryReqBody) => {
  const response = await FetchServer.Post<CategoryReqBody, Category>(categoryApiPaths.create, data);
  return response;
};
...
export const removeCategories = async (query: ApiQuery) => {
  const response = await FetchServer.Delete<any, any>(categoryApiPaths.remove + getApiQuery(query));
  return response;
};
